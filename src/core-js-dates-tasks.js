...
function getNextFriday(date) {
  ...
  const ms = nextDay * 24 * 3600 * 1000;

  return new Date(Date.parse(date) + ms);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const ms = Date.parse(dateEnd) - Date.parse(dateStart);
  return ms / 1000 / 3600 / 24 + 1;
}
...
function formatDate(date) {
  ...
  let hours = dateToFormat.getUTCHours();
  ...
  return `${month}/${day}/${year}, ${hours || 12}:${minutes}:${seconds} ${flag}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  do {
    ...
    i += 1;
  } while (i <= new Date(year, month, 0).getDate());
...
function getWeekNumberByDate(date) {
  ...
  const numberOfDaysInFirstWeek = 7 - (firstDay || 7) + 1;

  const diff = today - start;
  const oneDay = 24 * 3600 * 1000;
  const day = Math.ceil(diff / oneDay) + 1;

  return Math.ceil((day - numberOfDaysInFirstWeek) / 7) + 1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const start = Date.parse(reverse(period.start));
  const end = Date.parse(reverse(period.end));
  ...
  for (let i = start; i <= end; i += oneDay) {
    ...
    if (work < countWorkDays) {
      array.push(
        `${padStart(today.getDate())}-${padStart(today.getMonth() + 1)}-${today.getFullYear()}`
      );
      work += 1;
      if (work === countWorkDays) {
        weekend = 0;
      }
    } else if (weekend < countOffDays) {
      weekend += 1;
      if (weekend === countOffDays) {
        work = 0;
      }
    }
  }
...
function isLeapYear(date) {
  const year = date.getFullYear();
  const leapDay = new Date(year, 2, 0);
